...
const Keywords = ({id}: Props) => {
    //a custom hook has to be used which will fetch the keywords which are stored in the redux store
    ...
    <div className='bg-background-80 flex flex-col gap-y-3 p-3 rounded-xl'>
        ...
        <div className='flex flex-wrap justify-start gap-2 items-center'> 
            ...
            word.id !== latestVariable.variables.id && (
                <div key={word.id} className='bg-background-90 flex items-center gap-x-2 capitalize text-text-secondary py-1 px-4 rounded-full'> 
                    <p>{word.word}</p>
